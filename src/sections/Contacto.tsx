...
export default function Contacto() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isValidPhone(form.telefono)) {
      setFieldError('Ingresá un número de teléfono válido (7–20 dígitos, puede incluir guiones y paréntesis).');
      return;
    }
    if (form.email && !isValidEmail(form.email)) {
      setFieldError('Ingresá un email válido o dejá el campo vacío.');
      return;
    }
  ...
  const inputClass = 'w-full p-3 font-sans text-sm border outline-none transition-colors bg-transparent';
  const inputStyle = {
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div>
            {status === 'ok' ? (
              ...
              >
                ...
                <button
                  onClick={handleReset}
                  className="font-mono text-xs underline self-start"
                  style={{ color: 'var(--color-accent)', textUnderlineOffset: '3px' }}
                >
                  Enviar otro mensaje →
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
                ...
                <div>
                  <label htmlFor="contacto-empresa" className="sr-only">Empresa / Estudio</label>
                  <input
                    id="contacto-empresa"
                    type="text"
                ...
                <div>
                  <label htmlFor="contacto-telefono" className="sr-only">Teléfono</label>
                  <input
                ...
                {fieldError && (
                  <p className="font-sans text-xs" style={{ color: '#ef4444' }} role="alert">
                    {fieldError}
                  </p>
                )}
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              <div className="w-12 h-12 flex items-center justify-center shrink-0" style={{ backgroundColor: '#25D366' }}>
            ...
            <div
              ...
              style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
            >
              <p className="font-mono text-xs mb-3" style={{ color: 'var(--color-accent)' }}>[HORARIO]</p>
              <div className="flex flex-col gap-1">
                <p className="font-sans text-sm" style={{ color: 'var(--color-text)' }}>Lunes a Viernes: 8hs – 20hs</p>
                <p className="font-sans text-sm" style={{ color: 'var(--color-muted)' }}>Guardias para emergencias</p>
                <p className="font-sans text-sm" style={{ color: 'var(--color-muted)' }}>Zona: Córdoba Capital y Gran Córdoba</p>
              </div>
            </div>
